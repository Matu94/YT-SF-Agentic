...
1.  **Strict Execution Order**: Guaranteed by nested numerical prefixing across medallion layers and object types (e.g., `01_landing/01_infrastructure`).
...
## Decision
We implemented a custom Python script `.deployment/deploy.py` that relies on lexicographical sorting of the file tree. This allows for a hierarchical deployment strategy:
...
- **Compliance**: All new DDL must follow the nested prefixing pattern in the `snowflake/` directory:
    - **Pattern**: `snowflake/[Layer_Prefix]_[Layer_Name]/[Object_Prefix]_[Object_Type]/[Filename]`
    - **Example**: `snowflake/01_landing/01_infrastructure/01_json_format.sql`
