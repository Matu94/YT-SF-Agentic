...
CREATE OR REPLACE NETWORK RULE YT_SF_PROD.TECH.YOUTUBE_API_NETWORK_RULE
  TYPE = HOST_PORT
  MODE = EGRESS
...
CREATE OR REPLACE SECRET YT_SF_PROD.TECH.YOUTUBE_API_KEY_SECRET
  TYPE = GENERIC_STRING
...
CREATE OR REPLACE EXTERNAL ACCESS INTEGRATION YT_SF_PROD_YOUTUBE_API_INTEGRATION
  ALLOWED_NETWORK_RULES = (YT_SF_PROD.TECH.YOUTUBE_API_NETWORK_RULE)
  ALLOWED_AUTHENTICATION_SECRETS = (YT_SF_PROD.TECH.YOUTUBE_API_KEY_SECRET)
  ENABLED = TRUE
...
-- 1. Network Rule (DEV)
CREATE OR REPLACE NETWORK RULE YT_SF_DEV.TECH.YOUTUBE_API_NETWORK_RULE
  TYPE = HOST_PORT
  MODE = EGRESS
  VALUE_LIST = ('youtube.googleapis.com', 'www.googleapis.com')
...
-- 2. Secret (DEV)
--    !! REPLACE THE PLACEHOLDER BEFORE EXECUTING !!
CREATE OR REPLACE SECRET YT_SF_DEV.TECH.YOUTUBE_API_KEY_SECRET
  TYPE = GENERIC_STRING
  SECRET_STRING = 'YOUR_YOUTUBE_API_KEY_HERE'
...
CREATE OR REPLACE EXTERNAL ACCESS INTEGRATION YT_SF_DEV_YOUTUBE_API_INTEGRATION
  ALLOWED_NETWORK_RULES = (YT_SF_DEV.TECH.YOUTUBE_API_NETWORK_RULE)
  ALLOWED_AUTHENTICATION_SECRETS = (YT_SF_DEV.TECH.YOUTUBE_API_KEY_SECRET)
  ENABLED = TRUE
