...
*   **`STAGING.STG_YOUTUBE_CHANNEL_STATS` (Incremental Table)**
    ...
    *   **Calculations**: Calculates `daily_subscriber_growth` by subtracting yesterday's cumulative subscribers from today's subscribers:
        ```sql
...
*   **`STAGING.STG_YOUTUBE_VIDEO_STATS` (Incremental Table)**
    ...
    *   **Calculations**: Computes `daily_views`, `daily_likes`, and `daily_comments` using `LAG()` window functions over the video's history.
