...
def append_step_summary(content: str):
    """If running in GitHub Actions, append markdown to the step summary."""
...
def cmd_detect_changes(args):
    ...
    if base_ref == "snowflake":
        ...
        if not merge_base:
            ...
            for default_branch in ("dev", "prod"):
...
def cmd_seed(args):
    """Register existing files in Snowflake history without executing them."""
...
def main():
    ...
    p_detect.add_argument("--base-ref", default="dev",
                          help="Git ref to diff against (default: dev, use 'snowflake' for CI)")
    ...
    # seed
    p_seed = sub.add_parser("seed", help="Seed SQL files to Snowflake without execution")
    p_seed.add_argument("--file-list", default=OUTPUT_FILE,
                        help=f"Path to file list (default: {OUTPUT_FILE})")
    p_seed.add_argument("--branch", default=None, help="Branch name (auto-detected if omitted)")
    p_seed.add_argument("--commit", default=None, help="Commit SHA (auto-detected if omitted)")
